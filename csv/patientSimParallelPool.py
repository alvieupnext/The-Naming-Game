...
def getSimDataFromPatient(patient):
  ...
  print(f"Generating Simulation Patient Data {patient}")
...
def getDataFromPatient(patientList):
  ...
  for patient in patientList:
    ...
    patientRepeated = [patient for _ in range(totalSimulations)]
    consensusList = list(map(getSimDataFromPatient, patientRepeated))
    for sim, simValues in enumerate(consensusList):
      ...
      while len(reformattedSimValues) < len(consensusScoreList):
        reformattedSimValues.append(maxIterations)
...
if __name__ == "__main__":
  with Pool(5) as pool:
    ...
    print(patientData)
    patientData.to_csv("output/convergencePerPatient(N_back_Reduced)_weighted_4.csv")
